...
| 平台 | 状态 | 技术栈 |
|------|------|--------|
| macOS | ✅ v0.7.3 | Tauri 2.x + Rust + React 19 |
...
### macOS v0.7.3
- **修复 AI 解释生成静默失败**：默认/预设的 DeepSeek 模型是推理模型，回答前会先做一段内部"思考"，这段思考也计入 `max_tokens` 预算；原来设的 800 太小，思考就把预算耗尽，最终答案 `content` 变成空字符串——请求本身没有报错，只是"成功"生成了空结果，界面上什么都不显示，也没有错误提示。现在把 `max_tokens` 调到 3000（给思考+回答留够空间），并加了防御性检查：如果最终内容还是空的，会明确报错而不是静默无反应
- DeepSeek 预设模型名从 `deepseek-v4-flash` 更新为 `deepseek-flash`（DeepSeek 官方重命名）

### macOS v0.7.2
- **修复查词偶发跳转到"未找到"**：候选词列表的选词逻辑里，点击/方向键选词会同时触发两条并发的查词请求（一条来自点击处理函数，另一条来自监听 `selectedWord` 变化的 `useEffect`——因为前者内部也会更新 `selectedWord`，反过来又触发后者）。两条请求谁先返回不确定，若切词较快导致后到达的响应对应的是别的词，就会用错误结果覆盖刚显示出来的正确释义，界面表现为"明明查到了，突然又说未找到"。现在改为只有一个触发点发起查词，并加入请求令牌校验，过期的响应会被丢弃

### macOS v0.7.1
- **查词页标签弹层打磨**：加宽面板、hover 显示完整标签名（长标签名不再被截断看不全）、支持直接重命名标签、默认标签的星标常显（不用悬停才看到）
- **修复生词本标签弹层"双击改名"失效**：双击的第一次点击会先触发"选中并关闭弹层"，第二次点击落空——改用悬停可见的编辑图标触发改名，不再依赖双击手势
- 生词本标签弹层同步加宽、hover 显示完整标签名

### macOS v0.7.0
- **生词本标签筛选条重构**：条上只常驻默认标签 + 最近筛选过的标签（最多 5 个），其余标签、新建/重命名/删除/设默认都收进右侧固定的"标签"弹层（带搜索），解决标签一多就要滚很远才能点到"+"的问题
- **AI 解释支持手动保存**：生成后点"保存"才写入本地缓存，下次查同一个词直接显示，不用重新生成
...
- **删除防护**：Simulator 下 `Documents/dicts/` 实际指向 Mac 真机的 `~/.glean/dicts/`（与 macOS 版共享），因此 Simulator 环境屏蔽了删除词典操作，仅真机允许删除，避免误删 Mac 端文件

### v0.6.2
- **搜索框焦点修复**：收藏 / 发音 / 标签按钮不再抢占搜索框焦点，点击收藏后可直接继续打字查词，无需按 `/` 或手动点回输入框
- **词典释义链接可点击**：词条内部互相引用的链接此前点击无反应，现可跳转到对应词条；`http(s)` 链接改为用系统默认浏览器打开
